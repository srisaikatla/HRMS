...
const AttendancePage = () => {
    ...
    return (
      ...
      (entry.punchIn
        ...
        entry.punchOut
          ...
          .toLowerCase()
          .includes(searchQuery.toLowerCase()) ||
        `${entry.production.hours} hrs ${entry.production.minutes} mins ${entry.production.seconds} secs`
          .toLowerCase()
          .includes(searchQuery.toLowerCase()) ||
        `${entry.breakDuration.hours} hrs ${entry.breakDuration.minutes} mins ${entry.breakDuration.seconds} secs`
          .toLowerCase()
          .includes(searchQuery.toLowerCase()) ||
        `${entry.overtime} hrs`
          .toLowerCase()
  ...
  const totalHours = attendanceData.reduce(
    (sum, { production }) => sum + production.hours,
    0
  );
      ...
      <div className="flex flex-col md:flex-row gap-6 mb-8">
        <div className="bg-white p-6 rounded-lg shadow-lg w-full md:w-1/3">
          ...
          <div className="bg-[#0098F1] text-white p-4 rounded-lg mb-4">
            ...
            {isPunchedIn && (
              <p className="text-xl mt-2">
                Elapsed Time: {elapsedTime.hours} hrs {elapsedTime.minutes} mins{" "}
                {elapsedTime.seconds} secs
              </p>
            )}
          </div>
          ...
          <div className="flex justify-between mt-4 text-sm text-gray-600">
            <span className="border border-[#E65F2B] text-[#E65F2B] p-2 rounded-lg">
              Break {statistics.breakTime.hours} hrs{" "}
              {statistics.breakTime.minutes} mins {statistics.breakTime.seconds}{" "}
              secs
            </span>
            <span className="border border-[#E65F2B] text-[#E65F2B] p-2 rounded-lg">
        ...
        <div className="bg-white p-6 rounded-lg shadow-lg w-full md:w-1/3">
          ...
          {[
            {
              label: "Today",
              percentage: (totalHours / officeHours) * 100,
              color: "bg-blue-500",
              value: `${totalHours}/${officeHours}hrs`,
            },
            {
              label: "This week",
              percentage: (totalHours / (officeHours * 5)) * 100,
              color: "bg-green-500",
              value: `${totalHours}/${officeHours * 5}hrs`,
            },
            {
              label: "This month",
              percentage: (totalHours / (officeHours * 20)) * 100,
              color: "bg-purple-500",
              value: `${totalHours}/${officeHours * 20}hrs`,
            },
            {
              label: "Remaining",
              percentage:
                ((officeHours * 20 - totalHours) / (officeHours * 20)) * 100,
              color: "bg-yellow-500",
              value: `${officeHours * 20 - totalHours}/${officeHours * 20}hrs`,
            },
            {
              label: "Overtime",
              percentage: (statistics.overtime / (officeHours * 20)) * 100,
              color: "bg-red-500",
              value: `${statistics.overtime}hrs`,
            },
          ].map((stat, index) => (
          ...
          <div className="relative">
            ...
            {attendanceData.map((activity, index) => (
              <div className="flex items-start mb-8 ml-8" key={index}>
                <div
                  className={`w-3 h-3 bg-blue-500 rounded-full z-10 mt-1.5 pl-3 -ml-4`}
                ></div>
                <div className="ml-6">
                  <p className="text-sm text-gray-600">
                    {activity.punchIn.toLocaleString()}
                  </p>
                  ...
                  {activity.punchOut && (
                    <>
                      <p className="text-sm text-gray-600">
                        {activity.punchOut.toLocaleString()}
                      </p>
                      <p className="font-semibold flex text-[17px] items-center">
                  ...
                  <p className="text-sm text-gray-700">
                    {activity.production.hours} hrs{" "}
                    {activity.production.minutes} mins{" "}
                    {activity.production.seconds} secs
                  </p>
                  <p className="text-sm text-gray-700">
                    Break: {activity.breakDuration.hours} hrs{" "}
                    {activity.breakDuration.minutes} mins{" "}
                    {activity.breakDuration.seconds} secs
                  </p>
                  <p className="text-sm text-gray-700">
                    Overtime: {activity.overtime} hrs
                  </p>
                </div>
              </div>
            ))}
      ...
      <table className="min-w-full bg-white  rounded-lg shadow-lg border border-orange-900">
        ...
        <tbody>
          {filteredAttendanceData.map((entry, index) => {
            const { punchIn, punchOut, production, breakDuration, overtime } =
              entry;
            ...
            return (
              <tr key={index} className="border-t border-orange-500">
                ...
                <td className="py-2 px-4 text-orange-600 text-[14px] border border-orange-500">
                  {punchIn.toLocaleDateString()}
                </td>
                <td className="py-2 px-4 text-orange-600 text-[14px] border border-orange-500">
                  {punchIn.toLocaleTimeString()}
                </td>
                <td className="py-2 px-4 text-orange-600 text-[14px] border border-orange-500">
                  {punchOut ? punchOut.toLocaleTimeString() : "N/A"}
                </td>
                <td className="py-2 px-4 text-orange-600 text-[14px] border border-orange-500">
                  {production.hours} hrs {production.minutes} mins{" "}
                  {production.seconds} secs
                </td>
                <td className="py-2 px-4 text-orange-600 border text-[14px] border-orange-500">
                  {breakDuration.hours} hrs {breakDuration.minutes} mins{" "}
                  {breakDuration.seconds} secs
                </td>
                <td className="py-2 px-4 text-orange-600 border text-[14px] border-orange-500">
                  {overtime} hrs
                </td>
