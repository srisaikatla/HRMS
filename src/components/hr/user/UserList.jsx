...
const UserList = () => {
    ...
    const handleAddUser = () => {
        ...
        if (editUserId !== null) {
            ...
            // Add new user
            const newUserObject = {
                id: userdata.length > 0 ? Math.max(...userdata.map(user => user.id)) + 1 : 1,
                name: newUser.firstname + ' ' + newUser.lastname,
                email: newUser.email,
                status: newUser.status,
                createdDate: formattedDate,
    ...
    const openEditModal = (userId) => {
        ...
        let formattedDate = new Date(userToEdit.createdDate).toLocaleDateString('en-GB', {
            day: 'numeric',
            month: 'short',
            year: 'numeric'
        });
    ...
    return (
        <div className="p-5 flex flex-col h-screen">
            ...
            <div className="flex justify-end mb-4">
                <button className="bg-[#0098F1] text-white flex items-center rounded-lg px-6 py-3" onClick={() => setIsModalOpen(true)}>
            ...
            <div className="overflow-x-auto ">
                <table className="min-w-full">
                    <thead className="bg-[#0098F1] text-white">
                        <tr className="text-left">
                            <th className="text-xl py-2 px-4 bg-sky-100"></th>
                            <th className="text-xl text-white py-4 px-8 ">Name</th>
                            <th className="text-xl text-white py-2  px-8">E-mail Id</th>
                            <th className="text-xl text-white py-2 px-4 ">Status</th>
                            <th className="text-xl text-white py-2 px-4 ">Created Date</th>
                            <th className="text-xl text-white py-2 px-4 text-center">Role</th>
                            <th className="text-xl text-white py-2 px-8 ">Action</th>
                        </tr>
                    </thead>
                    <tbody>
                        {userdata.map(user => (
                            <ListItem key={user.id} user={user} onEdit={openEditModal} onDelete={handleDeleteUser} />
                        ))}
                    </tbody>
                </table>
            </div>
            ...
            {showSuccessMessage && (
                <div className="fixed top-0 left-0 w-full h-full flex items-center justify-center bg-black bg-opacity-50">
                    <div className="bg-sky-500 p-8 rounded-lg text-center text-white">
                        ...
                        <p>{editUserId ? 'User Updated Successfully' : 'User Added Successfully'}</p>
                    </div>
                </div>
            )}
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
                    <div className="bg-white rounded-lg p-8 w-full max-w-xl max-h-[600px] overflow-y-auto">
                        <h2 className="text-2xl mb-4 text-[#E65F2B] font-roboto">{editUserId ? 'Edit User' : 'Add List'}</h2>
                        <form className="grid grid-cols-2 gap-4 w-full">
                            <div className="flex flex-col space-y-4">
                                <label htmlFor="firstname" className="text-lg font-medium font-roboto">First Name</label>
                                <input
                                    type="text"
                                    id="firstname"
                                    name="firstname"
                            ...
                            <div className="flex flex-col space-y-4">
                                <label htmlFor="lastname" className="text-lg font-medium  font-roboto">Last Name</label>
                                <input
                                    type="text"
                                    id="lastname"
                                    name="lastname"
                            ...
                            <div className="flex flex-col space-y-4">
                                <label htmlFor="email" className="text-lg font-medium font-roboto">Email</label>
                                <input
                                    type="email"
                                    id="email"
                                    name="email"
                            ...
                            <div className="flex flex-col space-y-4">
                                <label htmlFor="mobile" className="text-lg font-medium font-roboto">Mobile Number</label>
                                <input
                                    type="text"
                                    id="mobile"
                                    name="mobile"
                            ...
                            <div className="flex flex-col space-y-4">
                                <label htmlFor="role" className="text-lg font-medium font-roboto">Role</label>
                                <input
                                    type="text"
                                    id="role"
                                    name="role"
                                    ...
                                    {Object.keys(selectedRoles).map(role => (
                                        <tr key={role}>
                                            ...
                                            <td className="px-4 py-2 text-center">
                                                <MdCheck
                                                    className={`h-8 w-8 p-2 rounded-full cursor-pointer ${selectedRoles[role].read ? 'bg-[#2A8F4C] text-white' : 'bg-[#CCCCCC] text-gray-500'}`}
                                                    onClick={() => toggleSelection(role, 'read')}
                                                />
                                            </td>
                                            <td className="px-4 py-2 text-center">
                                                <MdCheck
                                                    className={`h-8 w-8 p-2 rounded-full cursor-pointer ${selectedRoles[role].write ? 'bg-[#2A8F4C] text-white' : 'bg-[#CCCCCC] text-gray-500'}`}
                                                    onClick={() => toggleSelection(role, 'write')}
                                                />
                                            </td>
                                            <td className="px-4 py-2 text-center">
                                                <MdCheck
                                                    className={`h-8 w-8 p-2 rounded-full cursor-pointer ${selectedRoles[role].delete ? 'bg-[#2A8F4C] text-white' : 'bg-[#CCCCCC] text-gray-500'}`}
                        ...
                        <div className="col-span-2   gap-4 mt-5  flex justify-center">
                            <button type="button" onClick={handleAddUser} className="bg-[#0098F1] text-white rounded-lg h-[50px] w-[200px]  text-xl flex justify-center items-center">
                                Save
                            </button>
                            <button type="button" onClick={closeModal} className="border border-[#0098F1] text-[#0098F1] rounded-lg h-[50px] text-xl w-[200px] flex justify-center items-center">
                                Cancel
                            </button>
                        </div>
...
const ListItem = ({ user, onEdit, onDelete }) => {
    ...
    return (
        <tr className="text-black ">
            ...
            <td className="py-4 px-8 text-xl">{user.name}</td>
            <td className="py-2 px-8 text-xl">{user.email}</td>
            <td className="py-2 px-4 text-xl">{user.status}</td>
            <td className="py-2 px-4 text-xl">{user.createdDate}</td>
            <td className="py-2 px-2 text-xl text-white text-center">
                <div style={{ backgroundColor: '#0098F1', padding: '5px', }}>{user.role}</div>
            </td>
            <td className="py-2 px-8 flex">
                <button className="bg-[#2A8F4C] text-white px-2 py-2 rounded-lg mr-2" onClick={handleEdit}>
                    <FiEdit className="text-2xl" />
                </button>
                <button className="bg-[#FF3636] text-white  px-2 py-2 rounded-lg" onClick={handleDelete}>
