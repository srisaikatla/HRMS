...
const chatPersons = 
  [
    { "id": 1, "name": "Alice", "photo": "src/assets/hr/profile/man.png", "online": true },
    { "id": 2, "name": "Bob", "photo": "src/assets/hr/profile/man.png", "online": false },
    { "id": 3, "name": "Charlie", "photo": "src/assets/hr/profile/man.png", "online": true },
    { "id": 4, "name": "David", "photo": "src/assets/hr/profile/man.png", "online": false },
        ...
        <div className="flex w-full my-8 gap-1">
          ...
          {filteredChatPersons.map(person => (
            <button onClick={() => handlePersonClick(person)}>
                <div key={person.id} className="cursor-pointer flex items-center py-2 border-b">
                  <img src={person.photo} alt={person.name} className="w-10 h-10 rounded-full mr-3" />
                  <div className="flex flex-col items-center">
                    <span>{person.name}</span>
                    <span className="flex justify-start items-center gap-1">
                    <span className={`ml-1 w-3 h-3 rounded-full ${person.online ? 'bg-[#1D7616]' : 'bg-[#FF9900]'}`}></span>
          ...
          <div className="bg-white flex justify-between items-center h-[60px] border-b-2">
                <div className="flex items-center p-2 py-1">
                  <img src={selectedPerson.photo} alt={selectedPerson.name} className="w-10 h-10 rounded-full mr-3" />
                  <div className="flex flex-col items-center">
                    <span>{selectedPerson.name}</span>
                    <span className="flex justify-start items-center gap-1">
                    <span className={`ml-1 w-3 h-3 rounded-full ${selectedPerson.online ? 'bg-[#1D7616]' : 'bg-[#FF9900]'}`}></span>
                      <span>{selectedPerson.online?'online':'offline'}</span>
                    </span>
                  </div>
                </div>
                <div className="flex flex-row gap-x-3 w-[175px] mr-6">
                  <button><LuCamera className="bg-orange-500 h-[32px] w-[35px] p-[4px] rounded text-white"/></button>
                  <button><BiVideo className="bg-orange-500 h-[32px] w-[35px] p-[4px] rounded text-white"/></button>
                  <button><FiSettings className="bg-orange-500 h-[32px] w-[35px] p-[6px] rounded text-white"/></button>
                  <button><IoMdHelpCircleOutline className="bg-orange-500 h-[32px] p-[4px] w-[35px] rounded text-white"/></button>
                </div>
          </div>
          <div>
          ...
          {messages.map((message, index) => (
            <div key={index} className={`flex gap-x-3 p-2 my-1 ${message.sender === "me" ? "text-left" : "text-right"}`}>
              <img src={selectedPerson.photo} className="w-[45px] h-[45px]"/>
              {message.file ? (
                        message.file.type.startsWith("image/") ? (
                          <img
                            ...
                            onClick={() => window.open(URL.createObjectURL(message.file))}
                          />
                        ) : (
                        ...
                        <div className={`inline-block px-4 py-2 rounded-lg ${message.sender === "me" ? "border border-[#0098F1] text-black" : "border border-[#0098F1] text-black"}`}>
                          {message.text}
                        </div>
                      )}
            </div>
