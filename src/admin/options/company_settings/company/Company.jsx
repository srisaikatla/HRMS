...
const NavigationItem = ({ item, isActive, onClick }) => (
    <h2
        ...
        className={`text-[#E65F2B] text-lg p-2 cursor-pointer ${
            isActive ? 'bg-gradient-to-r from-[#E65F2B] to-[#FFC252] text-white' : 'hover:bg-gray-100'
        }`}
        role="button"
        aria-label={item}
...
const Company = () => {
    ...
    return (
        <div className="min-h-screen bg-orange-100 p-5 flex flex-col">
            ...
            <div className="flex flex-1">
                <div className="w-full sm:w-1/4 p-4 flex flex-col">
                    <div className="relative flex-grow border-r-2 border-[#E65F2B]">
                        {['Work Locations', 'Departments', 'Cost Centers', 'Company Logo', 'Employee Feedback'].map((item) => (
                            <NavigationItem
                                key={item}
                                item={item}
                ...
                <div className="w-full sm:w-3/4 p-4 flex flex-col flex-grow">
                    ...
                    {activeLeftItem === 'Employee Feedback' && (
                        <EmployeeFeedback
                            logActivity={(message) => logActivity('Employee Feedback', message)}
                        />
                    )}
                </div>
            </div>
                            ...
                            {activities[activeLeftItem].map((activity, index) => (
                                <li key={index} className="mb-4 flex items-start space-x-3">
                                    ...
                                    <div className="flex-grow">
                                        <p className="font-semibold text-[#E65F2B]">{activity.adminName}</p>
