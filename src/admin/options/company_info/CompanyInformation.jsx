  ...
  return (
    <div className="max-w-full mx-auto p-4">
      ...
      <div className="p-4 flex flex-col items-start">
        <div>
          <h2 className='text-2xl font-semibold mb-6 ml-4 sm:ml-7'>SPYD Technology</h2>
          <div className="space-y-4 text-left rounded-lg bg-white py-4 sm:p-8 shadow sm:ml-8 overflow-auto">
            <div className='flex justify-end'>
              {editable ? (
                <div className="text-right mr-2 space-x-1">
                  <button onClick={handleSave} className="mr-0 md:mr-4 bg-[#E65F2B] font-semibold w-16 md:w-20 h-9 text-sm md:text-lg text-white rounded-md">Save</button>
                  <button onClick={handleCancel} className="text-[#E65F2B] font-semibold w-16 md:w-20 h-9 text-sm md:text-lg border border-[#E65F2B] rounded-md">Cancel</button>
                </div>
              ) : (
                <FiEdit className="cursor-pointer text-white text-2xl mr-3 rounded-md p-1.5 bg-[#E65F2B]" onClick={handleEdit} />
              )}
            </div>
            {['hrmsid', 'companyName', 'Tan', 'PAN', 'MainPhone', 'Industry', 'website'].map((field) => (
              <div key={field} className='flex flex-col sm:flex-row items-center md:items-start sm:ml-8'>
