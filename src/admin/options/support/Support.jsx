...
const Support = () => {
  ...
  const [designationDetails, setDesignationDetails] = useState({
    'Sathish Chiluveru (Account Owner)': { email: 'sathish@spydtech.com', phone: '+91 9912457579' },
    'Balaji (Sales Associate)': { email: 'balaji@spydtech.com', phone: '+91 8712457779' },
  });
  ...
  const handleInputChange = (e) => {
    ...
    setDesignationDetails((prevState) => ({
      ...prevState,
      [selectedDesignation]: { ...prevState[selectedDesignation], [name]: value }
    }));
  };

  return (
    <div className="mx-4 sm:mx-8 min-h-screen text-lg font-normal">
      <div className='flex justify-between items-center flex-wrap'>
        <div>
          <h2 className="text-xl font-bold mb-2">Admin</h2>
          <h3 className="text-lg font-semibold mb-8">Dashboard/Support Information</h3>
        </div>
        <button onClick={onclicking} className='bg-gradient-to-r focus:outline-none from-[#E65F2B] to-[#FFC252] px-6 sm:px-12 text-white font-semibold py-2 sm:py-3 rounded-xl'>
          Edit
        </button>
      </div>
      <div className='flex flex-wrap md:mt-0 mt-4 lg:flex-nowrap w-full'>
        <div className="flex-col w-full lg:w-3/5 gap-1">
          <div className='mb-9'>
            <h3 className='md:text-xl text-lg mb-3'>Sathish Chiluveru (Account Owner)</h3>
          ...
          <div>
            <h3 className='md:text-xl text-lg mb-3'>Balaji (Sales Associate)</h3>
            <h3 className='flex items-center gap-3 text-base md:text-lg underline mb-2'>
              <IoMdMail className='text-[#E65F2B]' />
        ...
        <div className='bg-white shadow-xl w-full lg:mt-0 mt-10 lg:w-[550px] px-4 py-6 sm:px-6 sm:py-8'>
          <form onSubmit={showsuccess}>
            <div className='flex flex-col sm:flex-row items-start sm:items-center justify-between px-2 sm:px-7 mb-4 sm:mb-7'>
              <label className='sm:mb-0 mb-2 md:text-lg text-base'>Subject</label>
              <input type='text' className="border-2 md:text-lg text-base border-[#E65F2B] rounded-md w-full sm:w-auto" />
            </div>
            <div className='flex flex-col sm:flex-row items-start sm:items-center justify-between px-2 sm:px-7 mb-4 sm:mb-7'>
              <label className='sm:mb-0 mb-2 md:text-lg text-base'>Explain it for us</label>
              <input type='text' className="border-2 md:text-lg text-base border-[#E65F2B] rounded-md w-full sm:w-auto" />
            </div>
            <div className='flex justify-center mt-6'>
              <button type='submit' className='bg-gradient-to-r focus:outline-none from-[#E65F2B] to-[#FFC252] px-8 sm:px-12 text-white font-semibold py-2 sm:py-3 rounded-xl'>
                Save
              </button>
            </div>
          </form>
        ...
        <div className='fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50'>
          <div className='bg-white p-6 rounded-lg shadow-lg w-[90%] sm:w-[600px]'>
            ...
            <div className='flex justify-end mt-6'>
              <button onClick={handleSave} className='bg-gradient-to-r focus:outline-none from-[#E65F2B] to-[#FFC252] px-6 sm:px-8 text-white font-semibold py-2 rounded-xl'>
                Confirm Save
              </button>
            </div>
          </div>
